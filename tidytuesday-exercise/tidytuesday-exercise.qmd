...
# Define function to classify geographical areas
classify_geographical_area <- function(latitude, longitude) {
  if (latitude >= 25 & latitude <= 50 & longitude >= -125 & longitude <= -67) {
    return("Northeast")
  } else if (latitude >= 24 & latitude <= 38 & longitude >= -97 & longitude <= -76) {
    return("Southeast")
  } else if (latitude >= 35 & latitude <= 49 & longitude >= -104 & longitude <= -93) {
    return("Midwest")
  } else if (latitude >= 31 & latitude <= 45 & longitude >= -107 & longitude <= -94) {
    return("Southwest")
  } else if (latitude >= 32 & latitude <= 49 & longitude >= -124 & longitude <= -114) {
    return("West")
  } else {
    return("Other")
  }
...
# Apply the function to generate the new variable
cleaned_eclipse_data <- cleaned_eclipse_data %>%
  mutate(area = mapply(classify_geographical_area, 
                                    lat, 
                                    lon))
...
# Initialize empty list to store bootstrap estimates
...
for (i in seq_along(lm_res)) {
  # Get the training data for the current fold
  fold_data <- analysis(lm_res[[i]])
  
  # Perform bootstrap resampling
  boot_results <- boot(data = fold_data, statistic = function(data, indices) {
    fit <- lm(time_duration_minutes ~ lon + lat, data = data[indices, ])
    coef(fit)  # Extract model coefficients
  }, R = num_bootstraps)
  
  # Store bootstrap results
  bootstrap_results[[i]] <- boot_results
}

# Calculate uncertainty from bootstrap results
# For example, you can calculate confidence intervals for model coefficients
# Here, we calculate 95% confidence intervals
confidence_intervals <- lapply(bootstrap_results, function(boot_results) {
  apply(boot_results$t, 2, function(coefs) quantile(coefs, c(0.025, 0.975)))
})

# Print confidence intervals for each coefficient
for (i in seq_along(confidence_intervals)) {
  cat("Confidence intervals for fold", i, ":\n")
...
```{r}
# Define the Random Forest Classifier model specification
rf_spec <- rand_forest() %>%
